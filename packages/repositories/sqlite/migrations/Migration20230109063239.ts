...
export class Migration20230109063239 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'create table `view` (`id` text not null, `table_id` text not null, `created_at` datetime not null, `updated_at` datetime not null, `name` text not null, `display_type` integer not null, `kanban_field_id` text null, `calendar_field_id` text null, `filter` json null, `field_options` json null, `fields_order` text null, constraint `view_table_id_foreign` foreign key(`table_id`) references `table`(`id`) on update cascade, primary key (`id`, `table_id`));',
    )
