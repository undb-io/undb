...
export class Migration20230323062409 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      "create table `ego_field` (`id` text not null, `created_at` datetime not null, `updated_at` datetime not null, `deleted_at` datetime null, `table_id` text null, `name` text not null, `description` text null, `system` integer not null default false, `required` integer not null default false, `type` text check (`type` in ('id', 'created-at', 'updated-at', 'auto-increment', 'string', 'email', 'color', 'number', 'percentage', 'date', 'select', 'bool', 'date-range', 'reference', 'tree', 'parent', 'rating', 'count', 'lookup', 'sum', 'average')) not null, `format` text null, `foreign_table_id` text null, `parent_field_id` text null, `tree_field_id` text null, `max` integer null, `count_reference_field_id` text null, `lookup_reference_field_id` text null, `sum_reference_field_id` text null, constraint `ego_field_table_id_foreign` foreign key(`table_id`) references `ego_table`(`id`) on delete cascade on update cascade, constraint `ego_field_foreign_table_id_foreign` foreign key(`foreign_table_id`) references `ego_table`(`id`) on delete set null on update cascade, constraint `ego_field_count_reference_field_id_foreign` foreign key(`count_reference_field_id`) references `ego_field`(`id`) on delete set null on update cascade, constraint `ego_field_lookup_reference_field_id_foreign` foreign key(`lookup_reference_field_id`) references `ego_field`(`id`) on delete set null on update cascade, constraint `ego_field_sum_reference_field_id_foreign` foreign key(`sum_reference_field_id`) references `ego_field`(`id`) on delete set null on update cascade, primary key (`id`));",
    )
