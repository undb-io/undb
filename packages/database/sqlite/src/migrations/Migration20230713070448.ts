...
export class Migration20230713070448 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'create table `undb_form` (`id` text not null, `created_at` datetime not null, `updated_at` datetime not null, `deleted_at` datetime null, `name` text not null, `fields` json not null, `table_id` text null, constraint `undb_form_table_id_foreign` foreign key(`table_id`) references `undb_table`(`id`) on delete cascade on update cascade, primary key (`id`));',
    )
