...
export class Migration20230407085626 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'create table `ego_attachment` (`id` text not null, `record_id` text not null, `created_at` datetime not null, `updated_at` datetime not null, `deleted_at` datetime null, `table_id` text not null, `mime_type` text not null, `name` text not null, `size` integer not null, `token` text not null, `extension` text not null, constraint `ego_attachment_table_id_foreign` foreign key(`table_id`) references `ego_table`(`id`) on update cascade, primary key (`id`, `record_id`));',
    )
    ...
    this.addSql('create index `ego_attachment_mime_type_index` on `ego_attachment` (`mime_type`);')
    ...
    this.addSql(
      'create table `ego_user` (`id` text not null, `created_at` datetime not null, `updated_at` datetime not null, `deleted_at` datetime null, `username` text not null, `password` text not null, primary key (`id`));',
    )
