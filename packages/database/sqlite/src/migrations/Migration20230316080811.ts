...
export class Migration20230316080811 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      "create table `ego_field` (`id` text not null, `created_at` datetime not null, `updated_at` datetime not null, `deleted_at` datetime null, `table_id` text null, `name` text not null, `description` text null, `system` integer not null default false, `required` integer not null default false, `type` text check (`type` in ('id', 'created-at', 'updated-at', 'auto-increment', 'string', 'email', 'color', 'number', 'date', 'select', 'bool', 'date-range', 'reference', 'tree', 'parent', 'rating', 'count')) not null, `format` text null, `foreign_table_id` text null, `parent_field_id` text null, `tree_field_id` text null, `max` integer null, `reference_field_id` text null, constraint `ego_field_table_id_foreign` foreign key(`table_id`) references `ego_table`(`id`) on delete cascade on update cascade, constraint `ego_field_foreign_table_id_foreign` foreign key(`foreign_table_id`) references `ego_table`(`id`) on delete set null on update cascade, constraint `ego_field_reference_field_id_foreign` foreign key(`reference_field_id`) references `ego_field`(`id`) on delete set null on update cascade, primary key (`id`));",
    )
    ...
    this.addSql('create unique index `ego_field_reference_field_id_unique` on `ego_field` (`reference_field_id`);')
