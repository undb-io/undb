...
describe('RecordSqliteQueryVisitor', () => {
  ...
  test('stringEqual', () => {
    ...
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` = \'value\'"')
    visitor.stringEqual(new StringEqual('fieldKey', null))
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` = \'value\' and `fieldKey` is null"')
  })

  test('stringContain', () => {
    visitor.stringContain(new StringContain('fieldKey', 'value'))
    expect(visitor.query).toMatchInlineSnapshot('"select * where fieldKey like \'%`value`%\'"')
    visitor.stringContain(new StringContain('fieldKey', null))
    expect(visitor.query).toMatchInlineSnapshot('"select * where fieldKey like \'%`value`%\' and `fieldKey` is null"')
  })
  ...
  test('numberEqual', () => {
    ...
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` = 1000"')
  })
  ...
  test('dateEqual', () => {
    ...
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` = \\"2022-03-02T00:00:00.000Z\\""')

    visitor.dateEqual(new DateEqual('fieldKey', null))
    expect(visitor.query).toMatchInlineSnapshot(
      '"select * where `fieldKey` = \\"2022-03-02T00:00:00.000Z\\" and `fieldKey` is null"',
    )
  })
  ...
  test('dateIsToday', () => {
    ...
    expect(visitor.query).toMatchInlineSnapshot(
      '"select * where `fieldKey` between \\"2022-03-02T00:00:00.000Z\\" and \\"2022-03-02T23:59:59.999Z\\""',
    )
  })

  test('null', () => {
    visitor.null(new NullSpecification('fieldKey'))
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` is null"')
  })
  ...
  test('selectEqual', () => {
    ...
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` = \'opt1\'"')

    visitor.selectEqual(new SelectEqual('fieldKey', null))
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` = \'opt1\' and `fieldKey` is null"')
  })

  test('selectIn', () => {
    visitor.selectIn(new SelectIn('fieldKey', ['opt1, opt2']))
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` in (\'opt1, opt2\')"')
  })
  ...
  test('boolIsTrue', () => {
    ...
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` = true"')
  })

  test('boolIsFalse', () => {
    visitor.boolIsFalse(new BoolIsFalse('fieldKey'))
    expect(visitor.query).toMatchInlineSnapshot('"select * where `fieldKey` = false"')
  })

  test('string not equal', () => {
    const spec = new StringEqual('fieldKey', 'value').not()
    spec.accept(visitor)
    expect(visitor.query).toMatchInlineSnapshot('"select * where not `fieldKey` = \'value\'"')
  })
})
