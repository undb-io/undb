...
export class Migration20230131010538 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      "create table `field` (`id` text not null, `created_at` datetime not null, `updated_at` datetime not null, `deleted_at` datetime null, `table_id` text null, `name` text not null, `system` text not null default false, `type` text check (`type` in ('id', 'created-at', 'updated-at', 'auto-increment', 'string', 'email', 'number', 'date', 'select', 'bool', 'date-range', 'reference', 'tree', 'parent')) not null, `parent_field_id` text null, `tree_field_id` text null, constraint `field_table_id_foreign` foreign key(`table_id`) references `table`(`id`) on delete cascade on update cascade, primary key (`id`));",
    )
