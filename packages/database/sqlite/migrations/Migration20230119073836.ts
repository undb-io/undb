...
export class Migration20230119073836 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      "create table `field` (`id` text not null, `created_at` datetime not null, `updated_at` datetime not null, `deleted_at` datetime null, `key` text not null, `table_id` text null, `name` text not null, `type` text check (`type` in ('string', 'number', 'date', 'select', 'bool', 'date-range', 'reference', 'tree')) not null, constraint `field_table_id_foreign` foreign key(`table_id`) references `table`(`id`) on delete cascade on update cascade, primary key (`id`));",
    )
