import { TableCompositeSpecification, WithTableFormId, WithTableViewId, type ITableQueryModel } from '@undb/core'
...
export class GetSharedTableQueryHandler implements IQueryHandler<GetSharedTableQuery, IGetSharedTableOutput> {
  ...
  private getSpec(target: IShareTarget): TableCompositeSpecification {
  ...
  async execute(query: GetSharedTableQuery): Promise<IGetSharedTableOutput> {
    ...
    const spec = this.getSpec(query.target)
