...
const createRecordAuditSpec = (name: string, timestamp: Date, id: string, detail: object | null = null) =>
  WithAuditId.create()
...
export const getAuditSpecsFromEvent = (event: RecordEvents): AuditSpecification[] => {
  return match(event)
    .with(
      ...
      (event) => [createRecordAuditSpec(event.name, event.timestamp, event.payload.id)],
    )
    .with(
      {
        name: 'record.updated',
    ...
    .with(
      ...
      (event) =>
        event.payload.records.map((record) => createRecordAuditSpec('record.created', event.timestamp, record.id)),
    )
    .with(
      {
      ...
      (event) =>
        event.payload.updates.map((update) => createRecordAuditSpec('record.updated', event.timestamp, update.id)),
    )
    .with(
      {
        name: 'record.bulk_deleted',
      },
      (event) => event.payload.records.map((r) => createRecordAuditSpec('record.deleted', event.timestamp, r.id)),
    )
    .exhaustive()
}
