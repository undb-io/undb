<script lang="ts">
  ...
  type Tables = GetIndexQuery$result["tables"]
  export let table: Tables[0]

  $: tableId = $page.params.tableId
  $: viewId = $page.params.viewId
  $: active = table?.id === tableId && !viewId
  $: views = table?.views.filter((v) => !v.isDefault) ?? []
</script>
  ...
  <Accordion.Item value={table.id} class="w-full border-0">
    <Accordion.Trigger asChild class="w-full">
      ...
      >
        ...
        <span class={cn("ml-auto text-xs font-normal", active ? "text-muted-foreground" : "text-muted-foreground")}>
          {table.recordsCount}
        </span>
      </a>
    </Accordion.Trigger>
