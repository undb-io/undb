...
import { NestRealtimeEventsHandler } from './events/realtime.events-handler.js'

@Controller()
export class RealtimeController {
  constructor(
    private readonly handler: NestRealtimeEventsHandler,
    @InjectPinoLogger(RealtimeController.name)
    private readonly logger: PinoLogger,
  ) {}

  @Sse('tables/:tableId/subscription')
  sse(@Param('tableId') tableId: string): Observable<MessageEvent> {
