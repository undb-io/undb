...
export const KanbanDateBoard: React.FC<IProps> = ({ table, field, records }) => {
  ...
  const groupDateRecords = (): Record<string, CoreRecord[]> =>
    groupBy(
      (record) =>
        record.values.getDateValue(field.name.value).mapOr<string>(NODATE_STACK_ID, (v) => {
          if (!v) return 'NO_DATE'
  ...
  const {
    ...
    onDragItemEnd: (e, overContainer) => {
      updateRecord.mutate({
        ...
        value: [{ name: field.name.value, value: overContainer === NODATE_STACK_ID ? null : overContainer }],
      })
    },
